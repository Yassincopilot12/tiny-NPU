// =============================================================================
// tb_onnx_smoke.cpp - MLP smoke test for Graph Mode
...
// Helpers
// =============================================================================
static std::string g_datadir = "build/graph";
...
int main(int argc, char** argv) {
    ...
    printf("=== ONNX Graph Mode Smoke Test ===\n");
    ...
    // ---- Debug: dump key SRAM locations ----
    {
        printf("  DEBUG: SRAM dump after graph execution:\n");
        // gemm1_out / relu1_out at 0x0220, 16 bytes
        printf("    gemm1/relu1 @0x0220:");
        for (int i = 0; i < 16; i++) printf(" %02x(%d)", sram0_read(0x0220 + i), (int8_t)sram0_read(0x0220 + i));
        printf("\n");
        // b1_bcast at 0x0240, 16 bytes
        printf("    b1_bcast @0x0240:");
        for (int i = 0; i < 16; i++) printf(" %02x(%d)", sram0_read(0x0240 + i), (int8_t)sram0_read(0x0240 + i));
        printf("\n");
        // b1 at 0x0230, 16 bytes
        printf("    b1 @0x0230:");
        for (int i = 0; i < 16; i++) printf(" %02x(%d)", sram0_read(0x0230 + i), (int8_t)sram0_read(0x0230 + i));
        printf("\n");
        // output at 0x02D0, 8 bytes
        printf("    output @0x02D0:");
        for (int i = 0; i < 8; i++) printf(" %02x(%d)", sram0_read(0x02D0 + i), (int8_t)sram0_read(0x02D0 + i));
        printf("\n");
        // b2_bcast at 0x02F0, 8 bytes
        printf("    b2_bcast @0x02F0:");
        for (int i = 0; i < 8; i++) printf(" %02x(%d)", sram0_read(0x02F0 + i), (int8_t)sram0_read(0x02F0 + i));
        printf("\n");
        // DDR output at IO_BASE
        printf("    DDR output @IO_BASE:");
        for (int i = 0; i < 8; i++) printf(" %02x(%d)", ddr_mem[DDR_GRAPH_IO_BASE + i], (int8_t)ddr_mem[DDR_GRAPH_IO_BASE + i]);
        printf("\n");
    }
    ...
    // ---- Compare golden ----
    {
        ...
        if (gf) {
            ...
            // Read manifest to find output location
            // For simplicity, assume output was DMA_STOREd to DDR_GRAPH_IO_BASE
            // and golden is the expected DDR content at that location

// =============================================================================
// tb_onnx_overlap_perf.cpp - Overlap Performance test for Graph Mode
// Tests SERIAL vs OVERLAP scheduling with 3-layer MLP (64x64 GEMMs).
// Loads program/tdesc/ddr_image from files, runs graph pipeline, handles DMA
// transfers in C++, compares output against golden.
// =============================================================================
#include <cstdio>
#include <cstdlib>
#include <cstdint>
...
// Helpers
// =============================================================================
static std::string g_datadir = "build/graph_overlap_perf";
...
int main(int argc, char** argv) {
    ...
    printf("=== ONNX Overlap Performance Test ===\n");
    ...
    dut->trace(tfp, 99);
    tfp->open("onnx_overlap_perf.vcd");
    ...
    // Set scheduler mode: 0 = SERIAL
    dut->scheduler_mode = 0;
    ...
    // ---- Run until done or timeout ----
    {
        ...
        while (timeout-- > 0 && !done) {
            ...
            if (dut->graph_done) {
                ...
                printf("  perf_gemm_cycles:     %u\n", dut->perf_gemm_cycles);
                printf("  perf_softmax_cycles:  %u\n", dut->perf_softmax_cycles);
                printf("  perf_dma_cycles:      %u\n", dut->perf_dma_cycles);
                printf("  perf_reduce_cycles:   %u\n", dut->perf_reduce_cycles);
                printf("  perf_math_cycles:     %u\n", dut->perf_math_cycles);
                printf("  perf_gather_cycles:   %u\n", dut->perf_gather_cycles);
                printf("  perf_slice_cycles:    %u\n", dut->perf_slice_cycles);
                printf("  perf_concat_cycles:   %u\n", dut->perf_concat_cycles);
                printf("  perf_ew_cycles:       %u\n", dut->perf_ew_cycles);
                printf("  perf_overlap_cycles:  %u\n", dut->perf_overlap_cycles);
                printf("  perf_stall_cycles:    %u\n", dut->perf_stall_cycles);
            }
